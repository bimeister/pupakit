...
export class TableDndController<T> implements TableFeatureController {
  ...
  private processPanStart(): Subscription {
    return this.eventBus
      ...
      .pipe(
        ...
        filter((event: TableEvents.PanStart) => event.pointerType === 'mouse'),
        ...
        switchMap(([targetCell, tableColumn]: [TableEventTargetCellData, TableColumn]) =>
          forkJoin([of(targetCell), of(tableColumn), tableColumn.isCurrentResizable$.pipe(take(1))])
        )
      )
      .pipe(
        filter(
          ([_targetCell, _tableColumn, isCurrentResizable]: [TableEventTargetCellData, TableColumn, boolean]) =>
            !isCurrentResizable
        ),
  ...
  private startColumnDragging(targetCell: TableEventTargetCellData, tableColumn: TableColumn): void {
    ...
    this.currentDraggableColumn.isCurrentDraggable$.next(true);
  ...
  private updateCurrentTargetCell(event: TableEvents.Pan): void {
    ...
    this.columnIdToColumnMap$.pipe(take(1)).subscribe((columnIdToColumnMap: Map<string, TableColumn>) => {
      ...
      this.currentTargetColumn?.isCurrentDragTarget$.next(false);
  ...
  private stopColumnDragging(): void {
    this.currentDraggableColumn?.isCurrentDraggable$.next(false);
