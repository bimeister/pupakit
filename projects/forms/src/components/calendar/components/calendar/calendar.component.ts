...
import { CalendarMode } from '../../../../declarations/enums/calendar-mode.enum';
...
export class CalendarComponent implements OnChanges, OnDestroy {
  @Input()
  public mode: CalendarMode = CalendarMode.Date;
  ...
  @Output()
  public readonly selectDate: EventEmitter<Date[]> = new EventEmitter<Date[]>();
  ...
  public readonly dateTimeMode: CalendarMode = CalendarMode.DateTime;
  ...
  public ngOnChanges(changes: ComponentChanges<this>): void {
    if (changes.hasOwnProperty('mode')) {
      this.processModeChange(changes.mode);
    }
  ...
  private processModeChange(change: ComponentChange<this, CalendarMode>): void {
    const mode: CalendarMode = change.currentValue;
    if (isNil(mode)) {
      return;
    }

    if (mode === CalendarMode.Date) {
      this.calendarStateService.setStartTime(null);
      this.calendarStateService.setFinishTime(null);
    }
  }

  private processIsRangeChange(change: ComponentChange<this, boolean>): void {
    const isRange: boolean = change.currentValue;
    if (isNil(isRange)) {
      return;
    }

    this.calendarStateService.setIsRange(isRange);
  }
  ...
  private emitSelectDatesWhenSelectionChanged(): Subscription {
    return this.calendarStateService.resultDates$
      .pipe(distinctUntilSerializedChanged())
      .subscribe((selectedDateTimes: Date[]) => this.selectDate.emit(selectedDateTimes));
  }
}
