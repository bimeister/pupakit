...
export class TextareaStateService<T> {
  public readonly currentValue$: BehaviorSubject<string> = new BehaviorSubject('');

  public readonly currentValueLength$: Observable<number> = this.currentValue$.pipe(
    filterNotNil(),
    map((currentValue: string) => currentValue.length)
  );
