FROM dockerhub.bimeister.io/proxy_cache/library/node:16-slim as static-files
WORKDIR /temporary
...
COPY .npmrc package.json package-lock.json tsconfig.lib.json tsconfig.spec.json tsconfig.json npm-checklist.json build/ ./raw-copy/
